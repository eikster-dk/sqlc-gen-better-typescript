...
import (
	...
	"github.com/eikster-dk/sqlc-gen-better-typescript/cmd/effect/internal/version"
...
func (e *Effect4) generateRepositoryFile(tmpl *template.Template, filename string, queryViews []QueryView, sqlcVersion string) (toolbelt.File, error) {
	repoName := e.filenameToRepoName(filename)
	data := RepositoryData{RepositoryName: repoName, RepositoryNameCamel: toCamelCase(repoName), Filename: filename, QueryViews: queryViews, Imports: e.buildRepositoryImports(repoName, queryViews), SqlcVersion: sqlcVersion, PluginVersion: version.Version}
	content, err := executeTemplate(tmpl, data)
	if err != nil {
		return toolbelt.File{}, fmt.Errorf("failed to render repository template: %w", err)
	}
	return toolbelt.File{Name: fmt.Sprintf("%s.ts", repoName), Content: []byte(content)}, nil
}

func (e *Effect4) generateRequestFile(tmpl *template.Template, repoName string, queryViews []QueryView, sqlcVersion string) (toolbelt.File, error) {
	data := RequestData{RepositoryName: repoName, QueryViews: queryViews, Imports: e.buildRequestImports(queryViews), SqlcVersion: sqlcVersion, PluginVersion: version.Version}
	content, err := executeTemplate(tmpl, data)
	if err != nil {
		return toolbelt.File{}, fmt.Errorf("failed to render request template: %w", err)
	}
	return toolbelt.File{Name: fmt.Sprintf("%sRequest.ts", repoName), Content: []byte(content)}, nil
}

func (e *Effect4) generateResponseFile(tmpl *template.Template, repoName string, queryViews []QueryView, sqlcVersion string) (toolbelt.File, error) {
	data := ResponseData{RepositoryName: repoName, QueryViews: queryViews, Imports: e.buildResponseImports(queryViews), SqlcVersion: sqlcVersion, PluginVersion: version.Version}
	content, err := executeTemplate(tmpl, data)
	if err != nil {
		return toolbelt.File{}, fmt.Errorf("failed to render response template: %w", err)
	}
	return toolbelt.File{Name: fmt.Sprintf("%sResponse.ts", repoName), Content: []byte(content)}, nil
}
...
func (e *Effect4) generateModelsFile(tmpl *template.Template, catalog *models.Catalog, queryViewsByFile map[string][]QueryView, sqlcVersion string) (toolbelt.File, error) {
	...
	data := ModelsData{Imports: buildModelsImports(needsBigInt, needsExecRows), Enums: buildEnumViews(catalog.Enums), TableRows: e.buildTableRows(catalog, usedEmbedTables), NeedsBigInt: needsBigInt, NeedsExecRows: needsExecRows, SqlcVersion: sqlcVersion, PluginVersion: version.Version}
	content, err := executeTemplate(tmpl, data)
	if err != nil {
...
func (e *Effect4) localImportPath(path string) string {
	...
	if ext == "" || !strings.HasPrefix(path, "./") || strings.HasSuffix(path, ".js") || strings.HasSuffix(path, ".ts") {
		return path
	}
	return path + ext
}
