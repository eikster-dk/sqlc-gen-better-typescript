...
func (e *Effect4) buildQueryView(q models.Query, log *logger.Logger) QueryView {
	...
	if useTemplateLiterals {
		if hasParams {
			...
			if err != nil {
				log.Warn("Failed to transform SQL to template literal, falling back to original", logger.F("query", q.Name), logger.F("error", err.Error()))
...
func (e *Effect4) buildEmbedGroup(group models.EmbedGroup, queryName string) EmbedGroupView {
	...
	return EmbedGroupView{TableName: group.TableName, FieldName: fieldName, RowSchema: toPascalCase(group.TableName) + "Row", SchemaName: schemaName, Fields: fields, FieldMapping: fieldMappings}
}
