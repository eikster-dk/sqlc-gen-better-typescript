...
func TestNative_Build_GeneratesQueryFiles(t *testing.T) {
	...
	files, err := buildNative(n, defaultCatalog(), queries, log, "1.0.0")
	if err != nil {
...
func TestNative_Build_QueriesFile_SQLComment(t *testing.T) {
	...
	t.Run("multiline SQL has each line commented", func(t *testing.T) {
		...
		files2, err := buildNative(n, defaultCatalog(), []models.Query{multilineQuery}, log, "1.0.0")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		var qf *File
...
func TestNative_Build_OneQuery_NoParamQuery(t *testing.T) {
	...
	files, err := buildNative(n, defaultCatalog(), []models.Query{noParamQuery}, log, "1.0.0")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNative_Build_EmptyQuerySlice(t *testing.T) {
	...
	files, err := buildNative(n, defaultCatalog(), []models.Query{}, log, "1.0.0")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNative_Build_NilImportExtension(t *testing.T) {
	cfg := config.Config{Driver: "pg", Validator: "zod"}
	n := New(cfg)
	log := logger.New(false)
...
func TestNative_Build_RewrittenSQLFallback(t *testing.T) {
	...
	files, err := buildNative(n, defaultCatalog(), []models.Query{q}, log, "1.0.0")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNative_BuildParamList(t *testing.T) {
	...
	t.Run("single param", func(t *testing.T) {
		params := []models.SQLParameter{{FieldName: "id", Position: 1}}
	...
	t.Run("multiple params", func(t *testing.T) {
		params := []models.SQLParameter{
			{FieldName: "id", Position: 1},
			{FieldName: "name", Position: 2},
		}
		got := buildParamList(params)
...
func TestNative_Build_QueryImportExtension(t *testing.T) {
	cfg := config.Config{Driver: "pg", Validator: "zod", ImportExtension: ".ts"}
	n := New(cfg)
	log := logger.New(false)
...
func TestNative_Build_EnumUnionGeneration(t *testing.T) {
	...
	files, err := buildNative(n, catalog, []models.Query{q}, log, "1.0.0")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNative_Build_EnumUnion_EdgeCases(t *testing.T) {
	...
	t.Run("enum not in catalog falls back to z.string()", func(t *testing.T) {
		...
		files, err := buildNative(n, emptyCatalog, []models.Query{q}, log, "1.0.0")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		var rf *File
