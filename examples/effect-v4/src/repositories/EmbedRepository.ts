...
WHERE orders.id = ${params.id}`
  ...
  const getOrderWithCustomerEmbed = (params: GetOrderWithCustomerEmbedParams) =>
    getOrderWithCustomerEmbedRaw(params).pipe(
      Effect.map(Option.map(mapGetOrderWithCustomerEmbedRowToResult))
    )
...
ORDER BY orders.created_at DESC`
  ...
  const listOrdersWithCustomerEmbed = () =>
    listOrdersWithCustomerEmbedRaw().pipe(
      Effect.map((rows) => rows.map(mapListOrdersWithCustomerEmbedRowToResult))
    )
