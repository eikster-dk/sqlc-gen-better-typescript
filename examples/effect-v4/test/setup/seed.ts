...
export const seedDatabase = Effect.gen(function* () {
  ...
  yield* sql.unsafe(`SELECT setval('customers_id_seq', 10)`)
  ...
  yield* sql.unsafe(`SELECT setval('products_id_seq', 20)`)
  ...
  yield* sql.unsafe(`SELECT setval('orders_id_seq', 15)`)
  ...
  yield* sql.unsafe(`SELECT setval('order_lines_id_seq', 30)`)
})
