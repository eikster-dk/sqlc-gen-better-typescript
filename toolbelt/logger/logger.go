...
// Logger provides structured in-memory logging for debug artifacts.
type Logger struct {
	enabled bool
	entries []LogEntry
}
...
func New(enabled bool) *Logger {
	return &Logger{enabled: enabled, entries: make([]LogEntry, 0)}
}
...
func (l *Logger) log(level, msg string, fields []Field) {
	...
	l.entries = append(l.entries, LogEntry{Timestamp: time.Now(), Level: level, Message: msg, Fields: fieldMap})
}
